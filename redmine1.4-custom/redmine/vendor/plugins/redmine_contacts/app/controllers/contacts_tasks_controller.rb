class ContactsTasksController < ApplicationController
  ...
  def close
    ...
    respond_to do |format|
      format.js do 
        render :update do |page|  
            page["issue_#{params[:issue_id]}"].select('a').first().toggleClassName('closed');
            page["issue_#{params[:issue_id]}"].visual_effect :fade unless RedmineContacts.settings[:show_closed_issues]
        end
      end     
      format.html {redirect_to :back }
    end
